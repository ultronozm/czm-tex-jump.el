...
References are also followed through =\input= and =\include= files, using
RefTeX’s document file list.  In child files, set =TeX-master= to the master
document (for example, ="../main"=).  Citation jumps also use the document’s
bibliography declarations, with paths resolved relative to the master.
If a label or citation lookup fails using cached RefTeX information, the
package refreshes that information and retries once.  External labels are
checked before refreshing.
